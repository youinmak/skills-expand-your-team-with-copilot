...
def get_activities(
    ...
    - difficulty: Filter by difficulty level ('Beginner', 'Intermediate', 'Advanced').
                  Activities without a difficulty field are included in all levels.
    """
    ...
    if difficulty:
        # Include activities with the specified difficulty OR those with no difficulty set
        difficulty_condition = {"$or": [
            {"difficulty": difficulty},
            {"difficulty": {"$exists": False}}
        ]}
        if query:
            # Combine existing conditions with difficulty filter using $and
            query = {"$and": [query, difficulty_condition]}
        else:
            query.update(difficulty_condition)
